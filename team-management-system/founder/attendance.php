<?php
/**
 * Founder — Company-Wide Attendance
 */
...
// Count total records
$countStmt = $db->prepare("SELECT COUNT(*) FROM attendance a WHERE {$whereClause}");
...
$stmt = $db->prepare("
    SELECT a.*, u.name, u.email, u.role, u.designation
    FROM attendance a 
...
$employees = $db->query("SELECT id, name FROM users WHERE role IN ('employee','manager') AND status = 'active' ORDER BY name")->fetchAll();
...
<div class="page-header">
    <div>
        ...
        <p class="page-subtitle">Company-wide attendance tracking</p>
    </div>
</div>
...
<form method="GET" class="filter-bar">
    ...
    <?php if ($filterEmployee || $filterDate !== today()): ?>
        <a href="<?php echo BASE_URL; ?>/founder/attendance.php" class="btn btn-sm btn-outline">Reset</a>
    <?php endif; ?>
</form>
