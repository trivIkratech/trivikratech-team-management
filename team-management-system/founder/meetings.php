...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && post('form_action') === 'create_meeting') {
    ...
    if (empty($formErrors)) {
        try {
            ...
            header('Location: ' . BASE_URL . '/founder/meetings.php');
            exit;
...
// Fetch all active managers and employees to choose from
$usersList = $db->query("
    SELECT id, name, role, email 
    FROM users 
    WHERE status = 'active' AND role IN ('manager', 'employee')
    ORDER BY role ASC, name ASC
")->fetchAll();

// Fetch all meetings with host information
$stmt = $db->prepare("
    SELECT m.*, u.name AS host_name, u.role AS host_role
    FROM meetings m
...
<div class="page-header">
    <div>
        <h1 class="page-title">Meeting Planner</h1>
        <p class="page-subtitle">Schedule and manage corporate calls, team syncs, and one-on-ones</p>
    </div>
</div>
...
<div class="content-grid" style="display: grid; grid-template-columns: 1fr 2fr; gap: var(--space-6); align-items: start;">
    ...
    <div class="card fade-in">
        <div class="card-header">
            <h3 class="card-title">Schedule New Meeting</h3>
        </div>
        <form method="POST" action="" id="meeting-form">
            ...
            <div class="form-group">
                ...
                <input type="text" name="title" class="form-input" placeholder="e.g. Weekly Operations Sync" required value="<?php echo e(post('title')); ?>">
            </div>
            
            <div class="form-group">
                <label class="form-label">Description / Context</label>
                <textarea name="description" class="form-textarea" placeholder="Brief details about the meeting..." style="height: 80px;"><?php echo e(post('description')); ?></textarea>
            </div>
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <select name="meeting_type" id="meeting-type" class="form-select" required>
                        <option value="team" <?php echo post('meeting_type') === 'team' ? 'selected' : ''; ?>>Team Meeting</option>
                        <option value="individual" <?php echo post('meeting_type') === 'individual' ? 'selected' : ''; ?>>1-on-1 Sync</option>
            ...
            <div class="form-group">
                ...
                <textarea name="agenda" class="form-textarea" placeholder="Key points to discuss..." style="height: 100px;"><?php echo e(post('agenda')); ?></textarea>
            </div>
        ...
        <div class="card fade-in stagger-1">
            ...
            <?php if (empty($upcomingMeetings)): ?>
                <div class="empty-state" style="padding: var(--space-6) 0;">
                    ...
                    <div class="empty-state-text">Use the form on the left to schedule a new meeting.</div>
                </div>
            <?php else: ?>
