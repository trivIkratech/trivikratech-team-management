<?php
/**
 * Manager — My Teams & Squads Management
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && post('form_action') === 'edit_team') {
    ...
    if (!$team || ($team['leader_id'] != $managerId && $team['created_by'] != $managerId)) {
        $formErrors[] = 'Permission denied to edit this team.';
    } elseif (empty($name)) {
        $formErrors[] = 'Team name is required.';
    }
...
// Fetch manager teams
$teams = getManagerTeams($managerId);

// Fetch employees accessible to manager (direct team + other employees)
try {
    $myEmployees = $db->prepare("
        SELECT id, employee_id, name, email, role, designation, manager_id
        FROM users 
        WHERE status = 'active' AND role = 'employee'
        ORDER BY (manager_id = ?) DESC, name ASC
    ");
    $myEmployees->execute([$managerId]);
    $availableStaff = $myEmployees->fetchAll();
} catch (Throwable $e) {
    $availableStaff = [];
}
...
if (get('action') === 'edit' && get('id')) {
    $eTeam = getTeamById((int)get('id'));
    if ($eTeam && ($eTeam['leader_id'] == $managerId || $eTeam['created_by'] == $managerId)) {
        $editTeam = $eTeam;
...
<div class="page-header">
    <div>
        ...
        <p class="page-subtitle">Create squads from your employees and assign collaborative team tasks</p>
    </div>
...
<div class="stats-grid" style="margin-bottom: var(--space-6);">
    <div class="stat-card">
        ...
        <div class="stat-card-info">
            ...
            <div class="stat-card-label">My Active Teams</div>
        </div>
    </div>
    <div class="stat-card">
        ...
        <div class="stat-card-info">
            ...
            <div class="stat-card-label">Assigned Members</div>
        </div>
    </div>
    <div class="stat-card">
    ...
    <div class="card">
        <div class="empty-state" style="padding: 48px 20px;">
            ...
            <div class="empty-state-text" style="color: var(--color-text-muted); font-size: 13.5px; max-width: 420px; margin: 8px auto 20px;">
                Create squads for your projects (e.g. "Frontend Sprint", "SEO Campaign", "Support Group") to assign tasks to all members simultaneously.
            </div>
            <button type="button" class="btn btn-primary" onclick="openModal('createTeamModal')">
            ...
            <div class="card" style="display: flex; flex-direction: column; justify-content: space-between; border: 1px solid var(--color-border); border-radius: var(--radius-lg); background: var(--color-bg-card); transition: all var(--transition-fast);">
                    ...
                    <div style="background: var(--color-bg-secondary); border-radius: var(--radius-md); padding: 10px 12px; margin-bottom: 14px; display: flex; align-items: center; gap: 10px;">
                        ...
                        <div style="flex: 1; min-width: 0;">
                            ...
                            <div style="font-size: 13px; font-weight: 600; color: var(--color-text); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                                <?php echo e($team['leader_name']); ?> (You)
                            </div>
                        </div>
                    ...
                    <div style="margin-bottom: 14px;">
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 6px; max-height: 120px; overflow-y: auto; padding-right: 4px;">
                            <?php foreach ($members as $m): ?>
                                <a href="<?php echo BASE_URL; ?>/manager/tasks.php?assign_to=<?php echo $m['id']; ?>" 
                                   ...
                                   onmouseout="this.style.borderColor='transparent'; this.style.color='';">
                                    <span style="width: 6px; height: 6px; border-radius: 50%; background: <?php echo $m['id'] == $managerId ? '#a855f7' : 'var(--color-primary)'; ?>;"></span>
                                    <span><?php echo e($m['name']); ?></span>
                                    <?php if ($m['id'] == $managerId): ?>
                                        <small style="color: #a855f7; font-weight: 700;">(Lead)</small>
                                    <?php elseif (!empty($m['designation'])): ?>
                ...
                <div style="border-top: 1px solid var(--color-border); padding-top: 12px; margin-top: 8px; display: flex; justify-content: space-between; align-items: center; gap: 8px; flex-wrap: wrap;">
                    ...
                    <div style="display: flex; gap: 6px;">
                        <a href="<?php echo BASE_URL; ?>/manager/tasks.php?create_for_team=team_<?php echo $team['id']; ?>" class="btn btn-primary btn-sm" style="font-size: 12px; padding: 6px 12px;" title="Assign task to all team members at once">
        ...
        <form method="POST" action="">
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <input type="text" name="name" class="form-input" placeholder="e.g. Frontend Squad, Design Team, Sales Tigers" required>
                </div>
                
                <div class="form-group">
                    <label class="form-label">Description / Purpose</label>
                    <textarea name="description" class="form-textarea" rows="2" placeholder="Brief details about what this squad is working on..."></textarea>
                </div>

                <div class="form-group">
                    <label class="form-label">Select Team Members</label>
                    <div style="border: 1px solid var(--color-border); border-radius: var(--radius-md); max-height: 200px; overflow-y: auto; padding: 10px; background: var(--color-bg-secondary);">
                        <?php foreach ($availableStaff as $st): ?>
                            <?php $isDirect = ($st['manager_id'] == $managerId); ?>
                            <label style="display: flex; align-items: center; gap: 8px; padding: 6px 4px; cursor: pointer; border-bottom: 1px solid rgba(255,255,255,0.03); font-size: 13px;">
                                <input type="checkbox" name="members[]" value="<?php echo $st['id']; ?>" <?php echo $isDirect ? 'checked' : ''; ?>>
                                <span>
                                    <strong><?php echo e($st['name']); ?></strong> 
                                    <?php if ($isDirect): ?>
                                        <span class="badge badge-primary" style="font-size: 9px; padding: 1px 5px; margin-left: 4px;">My Team</span>
                                    <?php endif; ?>
                                    <?php if (!empty($st['designation'])): ?>
                                        <small style="color: var(--color-text-muted);">· <?php echo e($st['designation']); ?></small>
                                    <?php endif; ?>
                                </span>
                            </label>
                        <?php endforeach; ?>
...
<div class="modal-overlay active" id="editTeamModal">
    <div class="modal" style="max-width: 540px;">
        <div class="modal-header">
            ...
            <a href="<?php echo BASE_URL; ?>/manager/teams.php" class="modal-close">×</a>
        </div>
        <form method="POST" action="">
            <?php echo csrfField(); ?>
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <div style="border: 1px solid var(--color-border); border-radius: var(--radius-md); max-height: 200px; overflow-y: auto; padding: 10px; background: var(--color-bg-secondary);">
                        <?php foreach ($availableStaff as $st): ?>
                            <?php 
                            $isSelected = in_array($st['id'], $editTeamMembers) || ($st['id'] == $managerId); 
                            $isDirect = ($st['manager_id'] == $managerId);
                            ?>
                            <label style="display: flex; align-items: center; gap: 8px; padding: 6px 4px; cursor: pointer; border-bottom: 1px solid rgba(255,255,255,0.03); font-size: 13px;">
            ...
            <div class="modal-footer">
                <a href="<?php echo BASE_URL; ?>/manager/teams.php" class="btn btn-outline">Cancel</a>
