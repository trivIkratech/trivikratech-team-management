...
// =============================================
// User Online & Activity Tracking
// =============================================

/**
 * Update user's last activity timestamp for online/offline tracking
 */
function updateUserLastActivity(?int $userId = null): void {
    if (!$userId && isset($_SESSION['user_id'])) {
        $userId = (int)$_SESSION['user_id'];
    }
    if (!$userId) return;

    // Throttle to update at most once every 20 seconds per session
    $now = time();
    if (isset($_SESSION['last_activity_updated']) && ($now - (int)$_SESSION['last_activity_updated']) < 20) {
        return;
    }
    ...
    try {
        ...
        // Silently ignore if column doesn't exist or transient DB issue
    }
}

/**
 * Check if a timestamp indicates the user is online (within threshold, default 150 seconds / 2.5 min)
 */
function isUserOnline(?string $lastActivity, int $thresholdSeconds = 150): bool {
    if (empty($lastActivity)) return false;
    $time = strtotime($lastActivity);
    if (!$time) return false;
    return (time() - $time) <= $thresholdSeconds;
}

/**
 * Format user's last seen status string (e.g. "Online", "Last seen 5m ago", "Offline")
 */
function formatLastSeen(?string $lastActivity, int $thresholdSeconds = 150): array {
    $online = isUserOnline($lastActivity, $thresholdSeconds);
    if ($online) {
        return [
            'is_online' => true,
            'status_text' => 'Online',
    ...
    $ago = timeAgo($lastActivity);
    return [
        'is_online' => false,
        'status_text' => 'Offline',
        'last_seen_text' => 'Last seen ' . $ago
    ];
}

// Automatically touch activity on page requests if logged in
if (isset($_SESSION['user_id'])) {
    updateUserLastActivity((int)$_SESSION['user_id']);
}
