<?php
/**
 * HR — Attendance Management (Manage & Edit Employee Attendance)
 */
...
// Handle Edit / Manual Save Attendance by HR (HR can edit Employee attendance)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && post('form_action') === 'save_attendance') {
    ...
    setFlash('success', 'Attendance record for ' . $targetUser['name'] . ' on ' . formatDate($attDate) . ' saved successfully.');
    redirect(BASE_URL . '/hr/attendance.php?date=' . urlencode($attDate));
}
...
if (!empty($filterDate)) {
    ...
    $dataQuery = "
        SELECT a.id as attendance_id, a.date as attendance_date, a.check_in, a.check_out, a.break_start, a.break_end, a.total_break_time, a.total_working_time, a.status as att_status,
               u.id as user_id, u.name, u.email, u.designation, u.employee_id, m.name as manager_name,
               l.id as leave_id, l.leave_type, l.reason as leave_reason, l.status as leave_status
        FROM attendance a 
        JOIN users u ON a.user_id = u.id 
        LEFT JOIN users m ON u.manager_id = m.id
...
$pageTitle = 'HR — Employee Attendance';
include __DIR__ . '/../includes/header.php';
?>

<div class="page-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
    <div>
        <h1 class="page-title">Employee Attendance Management</h1>
        <p class="page-subtitle">Track, manage and edit workforce attendance records</p>
    </div>
    <div>
...
<form method="GET" class="filter-bar" style="display: flex; gap: 12px; flex-wrap: wrap; align-items: center; background: var(--color-bg-secondary); padding: 14px 18px; border-radius: var(--radius-md); border: 1px solid var(--color-border); margin-bottom: 20px;">
    ...
    <?php if ($filterEmployee || $filterStatus || ($filterDate !== today() && !empty($filterDate)) || empty($filterDate)): ?>
        <div style="margin-top: auto; padding-bottom: 2px;">
            <a href="<?php echo BASE_URL; ?>/hr/attendance.php" class="btn btn-sm btn-outline" style="height: 38px; display: inline-flex; align-items: center; gap: 6px;">
...
<?php if (empty($records)): ?>
    <div class="card">
        <div class="empty-state">
            ...
            <div class="empty-state-text">No employee attendance matches the selected filters.</div>
        </div>
    </div>
<?php else: ?>
    ...
    echo renderPagination($pagination, BASE_URL . '/hr/attendance.php?' . $qs);
    ?>
<?php endif; ?>

<!-- HR Edit / Manual Attendance Modal -->
...
function openEditAttendanceModal(data) {
    document.getElementById('modal_title_text').innerHTML = '<i class="fa-solid fa-pen-to-square"></i> Edit Attendance: ' + data.user_name;
