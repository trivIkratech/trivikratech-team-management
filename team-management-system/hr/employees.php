<?php
/**
 * HR — Employee Management (Add, Edit, and Delete Employees)
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && post('form_action') === 'edit_employee') {
    ...
    // Double check target user is employee
    ...
    if (empty($formErrors)) {
        $query = "UPDATE users SET employee_id = ?, name = ?, email = ?, contact_no = ?, designation = ?, status = ?, manager_id = ?, updated_at = NOW()";
        $params = [$employeeId, $name, $email, $contactNo ?: null, $designation ?: null, $status, $managerId];
...
// Fetch active managers list for dropdowns
...
<div class="page-header">
    <div>
        ...
        <p class="page-subtitle"><?php echo count($employees); ?> employee(s) registered in the system</p>
    </div>
    <div>
        <?php if ($action === 'list'): ?>
    ...
    <?php if (empty($employees)): ?>
        <div class="card">
            <div class="empty-state">
                ...
                <div class="empty-state-title">No employees registered</div>
                <div class="empty-state-text">Use the "+ Add Employee" button above to add employees to the system.</div>
            </div>
        </div>
    <?php else: ?>
                    ...
                    <?php foreach ($employees as $emp): ?>
                        <tr>
                            ...
                            <td style="text-align: right;">
                                ...
                                <a href="?delete=<?php echo $emp['id']; ?>" class="btn btn-ghost btn-sm" onclick="return confirm('Are you sure you want to delete this employee permanently?')" title="Delete Employee" style="color: var(--color-danger); padding: var(--space-1) var(--space-2); border-radius: var(--radius-sm); text-decoration: none; margin-left: var(--space-1);">
                                    🗑️ Delete
                                </a>
                            </td>
                        </tr>
